...
# =========================== [ADD YOUR TESTS HERE] ===========================
    ...
    def test_no_moves_after_win(self):
        '''Check if clicking a square after the game is done (winner is determined) adds an X.'''
        tiles = self.driver.find_elements(By.XPATH, self.BOARD_TILE_XPATH)
        tiles[0].click()  # X move
        tiles[3].click()  # O move
        tiles[1].click()  # X move
        tiles[4].click()  # O move
    ...
    def test_invalid_move(self):
        '''Check that a piece cannot be placed in an occupied square.'''
        tiles = self.driver.find_elements(By.XPATH, self.BOARD_TILE_XPATH)
        tiles[0].click()  # Place an X
        self.assertTileIs(tiles[0], self.SYMBOL_X)
        tiles[0].click()  # Attempt to place an O in the same spot
        self.assertTileIs(tiles[0], self.SYMBOL_X)  # Ensure it's still X 

    def test_neighbor_move(self):
        '''Check that a piece can only be moved up, down, left, right, or diagnoal'''
        tiles = self.driver.find_elements(By.XPATH, self.BOARD_TILE_XPATH)
        tiles[0].click()  # X move
        tiles[3].click()  # O move
        tiles[1].click()  # X move
        tiles[4].click()  # O move
        tiles[6].click()  # X moves 
        tiles[8].click()  # O moves
        #now X has to move a piece. If move could move to square 2, they would win
        #but this is not a valid move
        tiles[2].click()  # X tries to make an invalid move
        self.assertTileIs(tiles[2], self.SYMBOL_BLANK)  # Ensure square 2 it's still blank 
